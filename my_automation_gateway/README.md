...
Важливо для Windows/macOS: стартові скрипти спеціально шукають Python `3.12`, `3.11` або `3.13` і не створюють `.venv` на Python `3.14`, бо частина поточних залежностей ще може не мати готових Windows wheels для Python 3.14. Якщо існуючий `.venv` створений на непідтримуваній версії Python, скрипт перестворить його автоматично.
